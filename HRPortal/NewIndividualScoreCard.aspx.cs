    ...
    public partial class NewIndividualScoreCard : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                var query = nav.PerfomanceContractHeader.Where(r => r.Document_Type == "Individual Scorecard" && r.Approval_Status == "Released" && r.Score_Card_Type == "Departmental");
                List<Item> list = new List<Item>();
                foreach (var item in query)
                {
                    Item itm = new Item();
                    itm.Description = item.Description + "_" + item.Employee_Name;
                    itm.No = item.No;
                    list.Add(itm);
                }
                ...
                if (!String.IsNullOrEmpty(IndividualPCNo))
                {
                    ...
                    {
                        //strategicplanno.Text = item.CSP_Name;
                        ...
                        seniorOfficerPC.SelectedValue = item.Department_Center_PC_ID;
                    }
                }
            }
        }
        ...
        protected void revove_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (info[0] == "success")
                {
                    coreinitiativefeedback.InnerHtml = "<div class='alert alert-info'>" + info[1] + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
                }
                else
                {
        ...
        protected void savecoreacteivitiesdetails_Click(object sender, EventArgs e)
        {
            try
            {
                string flag = "";
                bool err = false;
                string IndividualPCNo = Request.QueryString["IndividualPCNo"];
                int lineno = Convert.ToInt32(clineno.Text);
                decimal nagreedtrgt = Convert.ToDecimal(cagreedtrgt.Text);
                string nremarks = cremarks.Text.Trim();
                string mduedate = returndate.Text.Trim();
                ...
                if(err)
                {
                    coreinitiativefeedback.InnerHtml = "<div class='alert alert-danger'>" +flag + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
                }
                else
                {
                    //String status = Config.ObjNav.FnSaveCoreInitiatives(IndividualPCNo, lineno, cduedate, nagreedtrgt, nremarks);
                    //String[] info = status.Split('*');
                    //if (info[0] == "success")
                    //{
                    //    coreinitiativefeedback.InnerHtml = "<div class='alert alert-info'>" + info[1] + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
